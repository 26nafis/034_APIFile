...
async function create(req, res) {
  try {
    ...
    if (genre_ids && genre_ids.length > 0) {
      const genres = await Genre.findAll({ where: { id: genre_ids } });
      await komik.setGenres(genres);
    }

    const result = await Komik.findByPk(komik.id, {
      include: [
        { model: Penulis, as: "penulis", attributes: ["id", "nama"] },
        { model: Genre, as: "genres", attributes: ["id", "nama"], through: { attributes: [] } }
      ]
    });

    return res.status(201).json({ message: "Komik berhasil ditambahkan.", data: result });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
}
...
async function update(req, res) {
  try {
    ...
    return res.status(200).json({ message: "Komik berhasil diperbarui.", data: result });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
}
